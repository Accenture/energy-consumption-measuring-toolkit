#include "rapl.h"
// Add the required header for file operations
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{ int fd;
  ...
  printf("\n");
      ...
      //package_threshold = read_config_value(filename);
...
{ int fd;
  ...
  /**
  The package domain in RAPL refers to the power consumption of the entire processor package. The processor package includes the processor cores, the memory controller, the I/O controller, and other peripherals.
  PP0 is the default RAPL domain, and it is the most commonly used. It is used to measure the power consumption of the processor cores.
  PP1 is less commonly used, but it can be used to measure the power consumption of specific devices in the uncore. For example, it can be used to measure the power consumption of the memory controller or the I/O controller.
  DRAM refer to the power consumption of the dynamic random access memory (DRAM) on the processor package. The DRAM is the main memory of the processor, and it is used to store data and instructions.
  */
  fd=open_msr(core);
...
void rapl_after(FILE * fp , int core, double package_threshold)
{ 
  int fd;
  long long result;
    if (package_threshold <= 0) {
           package_threshold = 500.0; // Default value
    }
  fd=open_msr(core);

  result=read_msr(fd,MSR_PKG_ENERGY_STATUS);
  package_after=(double)result*energy_units;
  //fprintf(fp,"Package after: %.6fJ consumed\n",package_after);
  //fprintf(fp,"Package before: %.6fJ consumed\n",package_before);
    //fprintf(fp,"Package energy: %.6fJ consumed\n",package_after-package_before);
  fprintf(fp,"%.3f,",package_after-package_before);  // PACKAGE - Total Power Consumption

  result=read_msr(fd,MSR_PP0_ENERGY_STATUS);
  pp0_after=(double)result*energy_units;

  fprintf(fp,"%.3f,",pp0_after-pp0_before);    // CORE
  ...
  (cpu_model==CPU_HASWELL)) {
     ...
     fprintf(fp,"%.3f,",pp1_after-pp1_before);     // GPU
  }
  else
    fprintf(fp,"NA(uncore),");

  if ((cpu_model==CPU_SANDYBRIDGE_EP) || (cpu_model==CPU_IVYBRIDGE_EP) ||
  (cpu_model==CPU_HASWELL)) {
     result=read_msr(fd,MSR_DRAM_ENERGY_STATUS);
    dram_after=(double)result*energy_units;
     fprintf(fp,"%.3f,",dram_after-dram_before);     // DRAM
  }
  else
    fprintf(fp,"NA(DRAM),");  
  fprintf(fp, "%.3fJ,",package_threshold);
  fprintf(fp, "%.3fJ,",package_after - package_before);
  
  
  //if (package_after-package_before >=  package_threshold ) {
      // Write the newline character to the file
    //  fprintf(fp, "\n");
    //  fprintf(fp, "Energy consumption threshold of %.2f J exceeded! Power consumption: %.2f J\n",
    //            package_threshold, package_after - package_before);
